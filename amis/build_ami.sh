...
#   os: the os to build (supported values: all|centos6|centos7|alinux|ubuntu1404|ubuntu1604|ubuntu1804)
...
do_command() {
    ...
    case ${_os} in
      ...
      centos6|centos7|alinux|ubuntu1404|ubuntu1604|ubuntu1804)
        packer build -color=false -var-file="${cwd}/packer_variables.json" -only=${only} --on-error=ask "${cwd}/packer_${_os}.json"
        RC=$?
        ;;
      *)
